...
data-model.md). The floor document (`GET /api/floor`) carries the same assembly as its
`attention` section, and the Desk counts down against that document's
`reference_instant`.

### What each surface carries — the settlement contract

The two surfaces are not the same list, and the difference is binding:

| Surface | Carries |
|---|---|
| `attention.items` in the floor document (`GET /api/floor`, and every `floor` SSE event) | **only unsettled items** — every item whose `settlement.state` is not `settled` |
| `GET /api/attention` | **every** stored item in rank order, settled ones included, at the end of the rank |

Why the split is stated here rather than left to the assembly: nothing in this data
model ever deletes a row (data-model.md invariant 5), and 002's Showfloor badge reads
`attention.items.length` as the count of things "waiting on you". A settled item left
in the floor document's section would be counted as waiting forever, and the operator's
one-second glance would be a lie. Settled items stay reachable — nothing is hidden,
nothing is deleted; they are read from `GET /api/attention`.

An item is `settled` only on factory truth: a Question whose `handle_relay` ruling was
`RESOLVED`, or any item whose factory read reports a non-null `resolution` (D-P8). A
press or a submit alone never settles anything, so nothing leaves `attention.items`
...
| `PANE_DEMO_RULING` | demo mode: which recorded `fixtures/bridge/<RULING>.json` a Question answer returns — one of the **five recorded** rulings `RESOLVED`, `ALREADY_RESOLVED`, `UNKNOWN`, `EXPIRED`, `UNAUTHORIZED` | `RESOLVED` |

No variable's value may appear in a page, an event, a log line, or a fixture.

### The recorded ruling documents

Each `fixtures/bridge/<RULING>.json` is `{"relay": {...}, "outcome": "<RULING>"}`; the
demo reader returns the `outcome` string and nothing else. Beside them,
`fixtures/bridge/malformed-relay.json` is `{"event": {...}, "relay": null,
"outcome": null}` — the adapter refused to build a relay, so no bridge call happened;
it is a refusal recording, not a ruling.

`SIGNAL_FAILED` is **not recorded**. It requires an orchestrator the signal cannot
reach, a state the capture could not stage, so no `fixtures/bridge/SIGNAL_FAILED.json`
exists. A `PANE_DEMO_RULING` naming a document that is not on disk is a degraded read
in words naming the missing path (001's loader rule) — never an invented ruling and
never a hand-written stand-in (constitution V, FR-018). The pane still derives
SIGNAL_FAILED at runtime, for an Escalation press whose signal RPC raised: that is an
observation, not a recording.
