...
- [ ] T006 [US1] In `pane/floor_document.py` (001's `assemble_floor_document`), **inside the per-epic join loop and nowhere else** — at the point where that epic's `workgraph.json` document and `epic_status` outcome are already in hand for the `NodeCard` join — call `assemble_stage(epic_id, workgraph_or_failure, live_outcome)` and attach the result as the `stage` key of that `EpicEntry` — additive: 001's `nodes`, `epic_state`, seams, and `scene` keys untouched, the Desk untouched; catch `json.JSONDecodeError` from the workgraph read there and pass it through as the failure (001 R-002 lets it propagate; 002 names it); no new reader, no new route, no new poll; one epic's failure never touches another's entry. **Concurrency note**: `ergane.yaml` declares `roadmap.max_concurrent_epics: 2` and 003-an-answer-reaches-the-factory carries the same `depends_on_landed: [001-the-desk-sees-the-floor]`, so it builds alongside this epic and no ordering edge separates them; its US1 (003 T008) is the other writer of this file — it rebuilds the document's **attention section** from `pane/attention.py`. Confine this diff to the epics loop plus the one `assemble_stage` import: touch no line of the attention assembly, the function signature, or the degraded roll-up, so the two hunks are disjoint and whichever epic lands second rebases cleanly in the merge queue (FR-002, FR-005) (spec US1-S1, US1-S6)
...
- [ ] T013 [P] [US1] Write `tests/test_stage.py::test_derived_flag_wins` over `fixtures/epic-status/paged/paged.json` read from disk as recorded: its node `us1`, carrying `awaiting_operator: true` and `state: "VERIFYING"`, stages with `waiting_on_operator` true and `state` still `"VERIFYING"` (spec US1-S7)
...
- [ ] T016 [P] [US2] Create `web/src/showfloor/types.ts` — `StageDocument`, `StagedNode`, `StagedEdge` (`kind: "pass" | "merge"`), `StageNote` — as the TypeScript mirror of contracts/stage-document.md §1 with every live field typed nullable, and add `stage?: StageDocument` to `EpicEntry` in 001's `web/src/api/floorDocument.ts` as **one line inside the `EpicEntry` declaration plus one `import type` line** — nothing else in that file changes, and 001's `AttentionItem` declaration and the attention section of the document type are left exactly as landed. **Concurrency note**: 003-an-answer-reaches-the-factory builds alongside this epic (`ergane.yaml` declares `roadmap.max_concurrent_epics: 2`; both specs carry only `depends_on_landed: [001-the-desk-sees-the-floor]`, and no cross-spec ordering edge exists), and its US1 (003 T010) is the other writer of this file — it extends `AttentionItem` with `kind: "notice"`, `actions`, `settlement`, a nullable `expires_at`, and a per-item `degraded`. Keep this hunk at the `EpicEntry` end of the file, as far from the attention types as the file allows, so the two diffs never overlap (spec US2-S3)
...
- [ ] T025 [US2] Create `web/src/showfloor/Showfloor.tsx` — the route: fetch `/api/floor` and subscribe to `/api/events` through 001's `web/src/api/events.ts` consumer exactly as `web/src/desk/Desk.tsx` does (one room is mounted at a time, so never a second `EventSource` or a second consumer) — call 001's two-argument signature as landed, `subscribeFloor("/api/events", setDoc)`, and neither fork the consumer, wrap it, nor re-declare its type: 003-an-answer-reaches-the-factory, concurrent with this epic, extends it to `subscribeFloor(url, onFloor, onAttention?)` with the third argument **optional**, so a plain two-argument call site keeps compiling under either landing order while a call site that passed a third argument, pinned the function's arity in a local type, or re-exported it would break under one of them; render `Masthead` with its `trailing` slot empty for now; when the document lists zero running epics render `[data-quiet-floor]` saying in words that the floor is quiet and mount no `[data-epic-stage]`; otherwise one `EpicStage` per `epics` entry in document order (FR-019); DESIGN.md § Navigation (Masthead: the mark, Desk / Showfloor nav, floor line; sticky at the top on the Showfloor), § Epic Timeline Row (Quiet floor: a hairline-bounded italic line in tinted slate says so; never a blank) (spec US2-S7)
...
- [ ] T033 [US2] Write `web/tests/smoke/showfloor.spec.ts` (against the `PANE_DEMO=1` backend 001's `playwright.config.ts` starts on 8787): open `/showfloor`; fetch `/api/floor` in the test and, for every `epics` entry, assert each `stage.nodes[i].id` has exactly one `[data-epic-stage][data-epic-id] [data-station][data-node-id]` carrying a non-empty `data-state`, and that at least one entry staged; assert at least one `.edge-pass` and one `.edge-merge` are present across the staged epics and differ in `data-edge-kind`; assert the legend strings of T031 are present (FR-018; SC-001). If the landed scene table in `pane/fixture_floor.py` stages no workgraph carrying both edge kinds, add one scene row binding `fixtures/workgraphs/077-a-scanner-the-operator-chooses-runs-in-the-loop.json` — the committed `fixtures/epic-status/` tree holds `002-expense-notes/`, `landing/`, `paged/`, `question/`, `refusal.json`, `refusal-exception.json` and `skew/` and records nothing for 077, so unless a recording lands first the honest fallback is the operative branch: stage 077 with its status read reported as the loader's missing-document `TransportFailed`, a `transport` note over a fully static graph, exactly what US1-S3 proves — editing code only, never a payload (plan.md § Risks and traps). **Concurrency note**: 003-an-answer-reaches-the-factory builds alongside this epic (`ergane.yaml` declares `roadmap.max_concurrent_epics: 2`; neither spec has an ordering edge on the other) and is the other writer of **both** files this task touches — `pane/fixture_floor.py` in its US1 (003 T004 seeds the attention store from `fixtures/webhook/*.json`), US2 (003 T022 adds `settle_question` / `press_escalation` to `FixtureReader`) and US3 (003 T040/T041 add `read_question` / `read_escalation_fate`); and `web/tests/smoke/showfloor.spec.ts` in its US4 (003 T058, which makes every committed smoke authenticate once the token gate closes). Confine the fixture-floor hunk to the one SCENES-table row and add no `Reader` method or `Settings` field; keep this smoke spec free of any token, credential, or `httpCredentials` literal so 003's gate closure is a `web/playwright.config.ts` change and never an edit here. Disjoint hunks are what let the merge queue rebase whichever epic lands second (spec US2-S1, US2-S4, US2-S5)
...
- [ ] T049 [US4] Create `web/src/showfloor/AttentionBadge.tsx` taking the floor document's `attention` section and its `degraded` entries for `section: "attention"`: when a degraded entry exists, render `[data-attention-degraded][data-mode="transport" | "refusal"]` whose text names what could not be learned (the attention items) and the mode in words, and render no count — the degraded entry wins whether or not items are also present, because a count taken from a read that failed is a number the pane cannot stand behind (constitution III); when `attention.items.length` is 0 render nothing; otherwise render exactly one `<a data-attention-badge href={DESK_PATH}>` whose text is the count (mono) followed by "waiting on you → Desk", with `DESK_PATH` imported from `web/src/routes.ts`, never retyped. The count stays `attention.items.length` and this component filters nothing: 003-an-answer-reaches-the-factory's `contracts/api.md` declares that the floor document's `attention.items` carries only **unsettled** items — settled ones are reachable through `GET /api/attention` — so the list's length is the number waiting on the operator both before and after that concurrent epic lands (FR-017); DESIGN.md § Navigation (the attention badge at the far right in clay-ink with a clay bell dot; it is a link, the Showfloor's only one), § Colors (The Attention Ranking Rule — clay for waiting-on-you; never red) (spec US4-S1, US4-S2, US4-S3)
...
- [ ] T052 [P] [US4] Write `web/tests/unit/AttentionBadge.test.tsx::count and target` (through `support/xyflow-double`, `fetch` stubbed, `EventSource` inert): render `Showfloor` against a floor document whose `attention.items` are the Fixture floor's recorded open items in 001's `AttentionItem` shape (built in the test from `fixtures/escalations/open_escalations.json` and `fixtures/webhook/question.json` via `?raw` imports — the README's committed paths — so N is the recording's open-item count, N > 0) and assert exactly one `[data-attention-badge]` whose text begins with `N` and whose `href` equals `/desk`; then the same document with `attention.items` replaced by `[]` at the document level (the recording itself is never emptied) and assert no `[data-attention-badge]` and no `[data-attention-degraded]` (spec US4-S1, US4-S2)
- [ ] T053 [P] [US4] Write `tests/test_showfloor_attention.py`: drive `assemble_floor_document` with a reader double whose `open_escalations()` raises `TransportFailed("attention")` in one run and `QueryRefused` in the other (the two fault shapes 001's `tests/test_degraded.py` injects at the seam) and assert exactly what the badge consumes and nothing more: the document's `degraded` list carries an entry with `section == "attention"` whose `mode` is `"transport"` in one run and `"refusal"` in the other, and the two entries differ by mode. Assert nothing about `attention.items` — 003-an-answer-reaches-the-factory is chartered to redesign that section (its US1 unions the pane's seeded store with the factory reads, its US3 attaches a per-item `degraded` and keeps stored items rendering when `open_escalations` fails), so an `items == []` clause would be this epic pinning a shape the concurrent sibling changes, and whichever landed second would turn the other's `uv run pytest -q` gate red. The badge reads the degraded entry, so the degraded entry is the whole contract this test exists to pin (spec US4-S3)
...
### Concurrent epic: 003-an-answer-reaches-the-factory

Not a dependency and not expressible as one. `ergane.yaml` declares
`roadmap.max_concurrent_epics: 2`, both specs carry exactly
`depends_on_landed: [001-the-desk-sees-the-floor]`, and the roadmap grammar has
no ordering edge between two specs — so once 001 lands, 002 and 003 dispatch
together and their nodes all branch from `dev`. Four files are written by both
epics; the merge queue can rebase them only if the hunks stay disjoint, so each
task below that touches one names its sibling writer:

| File | This epic writes | 003 writes |
|---|---|---|
| `pane/floor_document.py` | T006 — the `stage` key, inside the per-epic join loop | 003 T008 — the attention section, from `pane/attention.py` |
| `web/src/api/floorDocument.ts` | T016 — `stage?: StageDocument` on `EpicEntry` | 003 T010 — `AttentionItem` gains `kind: "notice"`, `actions`, `settlement`, nullable `expires_at`, `degraded` |
| `pane/fixture_floor.py` | T033 — at most one SCENES-table row (conditional) | 003 T004, T022, T040, T041 — store seeding and four `FixtureReader` seam halves |
| `web/tests/smoke/showfloor.spec.ts` | T033 creates it; T048 and T056 append cases | 003 T058 — makes every committed smoke authenticate once the token gate closes |

Two shapes are shared rather than shared files, and each is pinned where it is
consumed: 001's `subscribeFloor(url, onFloor)` gains an optional third argument
in 003, which T025 must keep compiling under either landing order; and the
floor document's `attention.items` carries only unsettled items per 003's
`contracts/api.md`, which is the declaration T049's count relies on and T053
...
- Four files carry a concurrent writer in 003-an-answer-reaches-the-factory (§ Concurrent epic): T006, T016, T033's fixture-floor row, and the smoke spec T033 creates. A hunk wider than its task describes is a merge-queue conflict for whichever epic lands second, not a matter of style. The same applies to shapes: T025 calls `subscribeFloor` with two arguments, and T053 asserts on the degraded entry only
