...
| **Attention badge** | `AttentionBadge` in `web/src/showfloor/AttentionBadge.tsx` | Rendered iff open attention items > 0, as one `<a>` whose text is the count and whose `href` is the Desk route (`/desk`, 001 R-010). Replaced by `[data-attention-degraded]` when the floor document's attention section degraded; never renders `0`. Count re-renders on every applied `floor` event. |
