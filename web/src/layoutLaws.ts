/**
 * The four layout laws, measured in one pass over a document.
 *
 ...
 *
 * ── why this file is under `src/` and not under `tests/` (011 US2, plan D2) ─
 *
 * 005 landed the measurement inside `showfloor.spec.ts`, 006 copied law (c) of
 * it into `desk.spec.ts`, and 009 moved it to `tests/smoke/support/laws.ts` so
 * both rooms imported one copy rather than carrying a third. 011 US2 moves it
 * one step further, out of the suite entirely, for a reason the suite cannot
 * satisfy: **the review room runs these four laws at runtime**, inside a
 * same-origin frame, in the operator's own browser (D-023, FR-008). Production
 * code cannot import from `web/tests/`.
 *
 * Nothing about the four changed in the move. What changed is the subject: the
 * measurement took the page's global `document` and now takes one, so it can be
 * pointed at a frame's document as easily as at its own. `measureLaws(page)` in
 * `tests/smoke/support/laws.ts` is this function evaluated in the page against
 * `document`, and the review room is this function called against
 * `frame.contentDocument`. **There is one implementation and there are two
 * callers** — a second answer to the same question is two answers that will
 * eventually disagree, and the whole value of the review room is that its
 * numbers are the numbers the gates measure.
 *
 * ── one self-contained function, deliberately ──────────────────────────────
 *
 * Every helper below lives *inside* `measureLawsIn`. That is not style: the
 * smoke suite serialises this function with `Function.prototype.toString()` and
 * evaluates it inside the browser, where nothing this module's scope holds
 * exists. A helper hoisted to module scope would be `undefined` in the page and
 * the whole suite would go dark. Keep it closed over nothing.
 */

/** What one pass over a document reports. */
 ...
 *
 * `doc` is the document under measurement and every read below is taken through
 * it — its `defaultView` for computed style and for the `DOMRect` constructor,
 * its own `elementsFromPoint` for the hit stack, its own `documentElement` for
 * the viewport. A frame's document therefore measures in the frame's own
 * coordinates and against the frame's own width, which is exactly what FR-007's
 * "at the selected width" means.
 */
export function measureLawsIn(doc: Document): LawReport {
  const view = doc.defaultView;
  if (view === null) {
    // A document with no view has no layout to measure, and a report of four
    // empty violation lists over one would be a pass nobody earned.
    throw new Error("the document has no view: there is nothing rendered to measure");
  }
  const styleOf = (element: Element): CSSStyleDeclaration => view.getComputedStyle(element);
