...
export default function EpicRow({ epic }: EpicRowProps) {
  ...
  return (
    <article className="epic" data-epic-id={epic.epic_id} data-scene={epic.scene ?? undefined}>
      <div className="epic-left">
        <span className="epic-name">{epic.epic_id}</span>
        <span className="epic-meta num">{epic.epic_state} · {epic.workflow_id}</span>
        {personas.length > 0 && <span className="epic-personas">{personas.join(", ")}</span>}
      </div>
      ...
      <div className="epic-readouts">
        ...
        <div className="ro">
          <span className="v num">{tracked?.attempt ?? <span className="unknown">unknown</span>}</span>
          <span className="k micro">attempts on {tracked?.story_key ?? tracked?.id ?? "—"}</span>
        </div>
        <div className="ro">
          <span className="v num"><span className="unknown">unknown</span></span>
