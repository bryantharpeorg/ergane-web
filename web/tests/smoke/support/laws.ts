/**
 * The layout laws, measured in one pass over a rendered page.
 *
 ...
 * smoke suite sweeps, in both rooms, so the measurement moved here and both
 * rooms import it rather than carrying a third copy. The code is 005's and
 * 006's, unedited except for law (d) and the one merge the two copies needed:
 * `describe()` carries both the Showfloor's `data-story-id` / `data-metric`
 * and the Desk's text snippet, and law (c) tells one leaf's line fragments
 * from another leaf's by the leaf's index rather than by that label, which is
 * what the label was standing in for.
 */
