...
describe("NodeChevron", () => {
  it("renders all eleven states distinctly", () => {
    ...
    for (const state of STATES) {
      act(() => {
        createRoot(container).render(
          <NodeChevron card={card(state)} />,
        );
      });

      const chev = container.querySelector(".chev");
      const className = chev?.className ?? "";
      const caption = chev?.querySelector(".cap")?.textContent ?? "";
      expect(caption).not.toBe("");
      pairs.add(`${className} | ${caption}`);
    ...
    // The set of (full className, caption) pairs has exactly eleven members.
    expect(pairs.size).toBe(11);
  });

  it("marks the paged VERIFYING node as paged, never WAITING_OPERATOR", () => {
    const container = document.createElement("div");
    document.body.appendChild(container);
    ...
    act(() => {
      createRoot(container).render(
        <NodeChevron
          card={card("VERIFYING", { awaiting_operator: true, id: "us1" })}
        />,
      );
    });
  ...
  it("renders a declared=false card as undeclared", () => {
    const container = document.createElement("div");
    document.body.appendChild(container);

    act(() => {
      createRoot(container).render(
        <NodeChevron card={card("VERIFYING", { declared: false })} />,
      );
    });
  ...
  it("keeps paged marker when also undeclared", () => {
    ...
    act(() => {
      createRoot(container).render(
        <NodeChevron
          card={card("VERIFYING", { declared: false, awaiting_operator: true })}
        />,
      );
    });
